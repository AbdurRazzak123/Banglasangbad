...
(function(){
  ...
  function loadNews(){
    if(!newsPromise) newsPromise=fetch(SHEET_URL+'?_='+Date.now(),{cache:'no-store'}).then(r=>r.json());
    return newsPromise;
  }
  ...
  function bind(){
    ...
    document.addEventListener('click',e=>{
      ...
      if(link.classList.contains('breaking-detail-btn') || /(?:^|\/)(?:details\.html|news\/[^?#]+\.html)(?:[?#]|$)/i.test(href)) return;
