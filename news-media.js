...
(function(){
 ...
 function run(){styles();fetch(NEWS_DATA_URL+'?_='+Date.now(),{cache:'no-store'}).then(r=>r.json()).then(list=>{let map=new Map(list.map(n=>[String(n.id),n]));let id=new URLSearchParams(location.search).get('id');if(id&&map.has(String(id)))detail(map.get(String(id)));}).catch(()=>{});}
