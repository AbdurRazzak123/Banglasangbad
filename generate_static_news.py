...
for a in articles:
    ...
    for x in others[:8]:
        ...
        thumb=x['image']
        im='<img class="detail-latest-thumb" src="../%s" alt="" loading="lazy">'%escape(thumb,quote=True).lstrip('./') if thumb else '<div class="detail-latest-thumb"></div>'
        latest.append('<a class="detail-latest-card" href="%s">%s<div class="detail-latest-info"><div class="detail-latest-cat">%s</div><div class="detail-latest-title">%s</div><div class="detail-latest-date">%s</div></div></a>'%(xpage,im,escape(x['category'] or 'সংবাদ'),escape(x['title']),escape(x['date'])))
