...
for a in articles:
    ...
    if im:
        ...
        tag = s.new_tag('img', src='', alt=a['title'], loading='eager', **{'data-news-id':a['id'], 'data-image-source':im})
    ...
    for idx in (1, 2):
        ...
        if ex:
            ...
            eim = s.new_tag('img', src='', alt=a['title'] + ' - ছবি ' + str(idx + 1), loading='lazy', **{'data-news-id':a['id'], 'data-image-index':str(idx), 'data-image-source':ex})
    ...
    for x in latest:
        ...
        if xim:
            xi = s.new_tag('img', src='', alt=x['title'], loading='lazy', **{'data-news-id':x['id'], 'data-image-source':xim}); th.append(xi)
    ...
    if grid:
        ...
        for x, label in six:
            ...
            xim = article_image(x, 0)
            if xim:
                xi = s.new_tag('img', src='', alt=x['title'], loading='lazy', **{'data-news-id':x['id'], 'data-image-source':xim}); box.append(xi)
            link.append(box)
